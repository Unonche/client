...
{#if isWebGLSupported()}
        ...
        <div class="p-4 container flex flex-col items-center" style="max-width: 100%">
          ...
          <div class="w-full md:w-2/4 bg-surface-900 rounded-lg text-center p-4 mb-2 shadow">
            ...
            <div class="flex">
              ...
              <button use:clipboard={window.location} on:click={onClipboard} class="btn-icon btn-icon-sm variant-filled-primary rounded-lg shrink-0">
                {#if copyingLink}
          ...
          {#if $room.sessionId === $gameState.kingPlayerId}
            <button class="btn btn-lg variant-filled-primary w-full md:w-2/4 shadow" class:opacity-50={$gameState.players.size < 2} class:cursor-not-allowed={$gameState.players.size < 2} on:click={actions.startGame}>
              <span>
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-6">
  ...
  {/if}
{:else}
  <div class="text-center">Pas de chance, WebGL n'est pas disponible sur ton navigateur</div>
{/if}
